...
class Boid:
    ...
    def run(self, grid, cols, rows, resolution):
        col = math.floor(self.position.x / resolution)
        row = math.floor(self.position.y / resolution)
        neighbors = []
        ...
        for i in range(-1, 2):
            for j in range(-1, 2):
                ...
                if newCol >= 0 and newCol < cols and newRow >= 0 and newRow < rows:
                    # Add all boids in this cell to neighbors
                    neighbors = neighbors + grid[newRow][newCol]
